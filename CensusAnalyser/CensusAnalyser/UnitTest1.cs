...
namespace CensusAnalyser
{
    public class Tests
    {
        ...
        {
            var ex = Assert.Throws<FileNotFoundException>(() => CSVStateCode.StateLoadData("Wrong_File_Path"));
            Assert.That(ex.Message, Is.EqualTo(CensusException.Wrong_File_Path.ToString()));
        }
        ...
        public void GivenFileExtansionIncorrect_WhenCheckFilePath_ShouldReturnsFileNotFoundException1()
        {
            ...
            var ex = Assert.Throws<FileNotFoundException>(() => CSVStateCode.StateLoadData(Path));
            Assert.That(ex.Message, Is.EqualTo(CensusException.Wrong_File_Path.ToString()));
        }
        ...
        {
            string Path = @"C:\Users\Srinidhi\source\repos\CensersAnaliserProblem\CensersAnaliserProblem\sateCode.csv";
            var ex = Assert.Throws<CensusAnalyserException>(() => CSVStateCode.CheckDelimiter(Path, "State,Population,AreaInSqKm,DensityPerSqKm"));
            Assert.That(ex.mgs, Is.EqualTo(CensusException.Wrong_Delimiter.ToString()));
        }
        ...
        {
            string Path = @"C:\Users\Srinidhi\source\repos\CensersAnaliserProblem\CensersAnaliserProblem\sateCode.csv";
            var ex = Assert.Throws<CensusAnalyserException>(() => CSVStateCode.CheckDelimiter(Path, "Wrong_Header"));
            Assert.That(ex.mgs, Is.EqualTo(CensusException.Wrong_Delimiter.ToString()));
        }
    }
}
